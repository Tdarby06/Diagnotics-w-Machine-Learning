...
```
PLAN  →  DESIGN  →  EXECUTE  →  STRATEGY  →  LAUNCH
```

1. **Plan** — Validate that FNA data + ML can improve diagnostic procedures
2. **Design** — Select a supervised learning algorithm maximizing accuracy, sensitivity, and specificity
3. **Execute** — Apply the trained model to new patient data, replacing or supplementing CNB procedures
4. **Strategy** — Continuously upload new patient data to servers to keep the model current and accurate
5. **Launch** — Deploy commercially via MedicML for faster, more cost-effective tumor diagnostics
...
### Opportunities
- Higher FNA specificity/sensitivity enables reduced cost, faster turnaround, and greater patient capacity
- Continuous data collection will compound model performance over time
- Positions MedicML as a leader in AI-assisted pathology diagnostics

### Risks
- **Data stagnation** — insufficient new data may cause model performance to plateau or degrade
- **Data diversity** — training data must represent diverse gender, ethnicity, background, and geography to ensure the model generalizes effectively across patient populations
